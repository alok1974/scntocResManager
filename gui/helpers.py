...
def _sortNames(inNames):
    ...
    for name in inNames:
        ...
        try:
            ...
            raise ValueError('cannot convert %s to a number !' % number)
...
def _extractRootAndSort(inNames):
    ...
    for modelName in inNames:
        ...
        if modelNumber is None:
            rootName = modelName
        else:
            rootName = modelName[:len(str(modelNumber)) * -1]

        if d not in rootName:
            d[rootName] = []
...
def _sortAndMakeColors(inModelNames):
    ...
    colorList = list(itertools.islice(
                            itertools.cycle(colorComb), None, len(rootNames)))

    rootColors = dict([
                        (rootName, colorList[index])
                        for index, rootName in enumerate(rootNames)
                      ])
    colorDict = {}
...
def _getCommonRes(inModelNameList, inModelAvResDict):
    return list(set.intersection(
        *map(set, [inModelAvResDict[modelName] for modelName in inModelNameList])))
